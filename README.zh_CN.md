...
- 停机指令：halt、end(停止程序并退出)两者在虚拟机上等效。（0x80，0xff）
...
9. 停机指令`halt`会让程序退出，并需要满足四字节对齐。每个程序都应当要在结尾处有`halt`指令，否则`pc`指向程序内存结尾后会异常退出。`end`也是与`halt`等效的，但汇编器在遇到`end`后会不再汇编后面的代码。所以汇编时只能在最后写`end`,而`halt`可以放在中间。

## 比特位

目前虚拟机和汇编器支持8位和16位。8位模式下，最多写 64 条指令，栈、内存、磁盘空间只有256字节。每条指令都长4字节。
16位模式下，寄存器为16位，栈、内存、磁盘空间有65536字节。每条指令都长8字节。
...
当汇编器运行时，第一遍会删除注释和空行，并检查是否满足4字节对齐。之后除了label标签和宏。第二遍会展开宏，并将汇编指令转换为二进制机器码。对于`nop`，`null`等值会直接写入0。汇编器遇到`end`后自动停止分析代码，所以`end`要写到最后面。
