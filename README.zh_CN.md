...
3. 立即数也可以通过`mov`实现，比如：`mov r0, 4 r0` 相当于把r0储存的值加入4。`mov r0, 0 r1`或则`mov r0, nop r1`是移动r0到r1。

4. 条件判断的指令为：指令 源1 源2 跳转目标。条件满足则会把`pc`的值设置为跳转目标。比如`always nop nop 0x08`无条件跳转到开头。`eq r1 r2 0x00`是判断r1和r2的值是否相等，相等则跳转到0x08。
...
8. call 指令格式为 call 目标地址 nop nop，功能是将下一条指令的地址压入栈中，并跳转到目标地址执行函数。比如 `call 0x20 nop nop` 是跳转到地址 0x20 执行函数，并保存返回地址。
...
目前虚拟机只支持8位，栈、内存、磁盘空间只有256字节。每条指令都长4字节。
